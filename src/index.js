...
const { createProgressiveTraceGenerator, createOHLCGenerator, createProgressiveRandomGenerator } = xydata
...
createProgressiveTraceGenerator()
    ...
    .then((data) => {
        ...
        line.add(data.map((point) => ({ x: point.x, y: point.y * 0.1 + 100 })))
    })
...
Promise.all([
    createProgressiveRandomGenerator().setNumberOfPoints(1000).generate().toPromise(),
    createProgressiveRandomGenerator().setNumberOfPoints(1000).generate().toPromise(),
]).then((data) => {
